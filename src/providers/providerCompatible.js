// providerCompatible.js — Universal OpenAI-Compatible provider with Cloudflare AI support (ESM, No classes)
import OpenAI from 'openai';
import { retryWithBackoff, streamWithBackoff } from './retry.js';

function createClient(options) {
  var rawBaseUrl = options.baseUrl || options.baseURL || '';
  return new OpenAI({
    baseURL: rawBaseUrl.replace(/\/+$/, ''),
    apiKey: options.apiKey || options.apikey || 'dummy',
...
export function chat(messages, options) {
  ...
  if (isStream) {
    ...
    function wrapCompatibleStream(evt) {
      streamEmitted = true;
      if (typeof options.onStream === 'function') {
        options.onStream(evt);
      }
    }
    return streamWithBackoff(function executeCompatibleStream() {
      return executeCompatibleChatStream(messages, options, wrapCompatibleStream);
    }, maxRetries, 1000, options.signal, function anyStreamChunk() {
      return streamEmitted;
    });
  }

  return retryWithBackoff(function executeCompatibleSync() {
    return executeCompatibleChatNonStream(messages, options);
  }, maxRetries, 1000, options.signal);
}

async function executeCompatibleChatStream(messages, options, onStream) {
...
async function executeCompatibleChatNonStream(messages, options) {
  var client = options.client || createClient(options);
  var body = {
    model: options.model,
    messages: convertMessagesOpenAI(messages),
...
export async function listModels(options) {
  options = options || {};
  var baseUrl = String(options.baseUrl || options.baseURL || '').replace(/\/+$/, '');
  var apiKey = options.apiKey || options.apikey || '';
  var headers = {};
  if (apiKey) headers['Authorization'] = 'Bearer ' + apiKey;

  // Cloudflare Workers AI parser
