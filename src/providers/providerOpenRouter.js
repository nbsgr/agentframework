// providerOpenRouter.js — OpenRouter provider adapter (ESM, No classes)
import OpenAI from 'openai';
import { retryWithBackoff, streamWithBackoff } from './retry.js';

function createClient(options) {
  var rawBaseUrl = options.baseUrl || options.baseURL || 'https://openrouter.ai/api/v1';
  return new OpenAI({
    baseURL: rawBaseUrl.replace(/\/+$/, ''),
    apiKey: options.apiKey || options.apikey || process.env.OPENROUTER_API_KEY || '',
    defaultHeaders: {
...
export function chat(messages, options) {
  ...
  if (isStream) {
    ...
    function wrapOpenRouterStream(evt) {
      streamEmitted = true;
      if (typeof options.onStream === 'function') {
        options.onStream(evt);
      }
    }
    return streamWithBackoff(function executeOpenRouterStream() {
      return executeOpenRouterChatStream(messages, options, wrapOpenRouterStream);
    }, maxRetries, 1000, options.signal, function anyStreamChunk() {
      return streamEmitted;
    });
  }

  return retryWithBackoff(function executeOpenRouterSync() {
    return executeOpenRouterChatNonStream(messages, options);
  }, maxRetries, 1000, options.signal);
}

async function executeOpenRouterChatStream(messages, options, onStream) {
...
async function executeOpenRouterChatNonStream(messages, options) {
  var client = options.client || createClient(options);
  var body = {
    model: options.model || 'anthropic/claude-3.5-sonnet',
    messages: convertMessagesOpenAI(messages),
